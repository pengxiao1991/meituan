		...
		$.ajax({
			...
			for(var pro in data){
				var index = data[pro].id;
					...
					$li.pro = pro;
				$li.jdLoad(function(){
					if (data[pro].nobespeak) {
						$li.find("div.fl").addClass("no");
					}
					$li.find("div.fl img")[0].src = "img/"+data[pro].src;
					$li.find("h3").html(data[pro].title);
					$li.find("div.fl").next().next("span").html(data[pro].description);
					$li.find("p>b").html(data[pro].price);
					$li.find("p>span").html(data[pro].oldprice);
					$li.find("p>i>span").html(data[pro].sales);
				});		
			}
