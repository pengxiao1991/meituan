...
/***/ function(module, exports) {
	...
	/*! Zepto 1.1.6 (generated with Zepto Builder) - zepto event ajax form ie assets callbacks data detect deferred gesture fx_methods fx touch stack selector ios3 - zeptojs.com/license */
	//     Zepto.js
		...
		$.ajax({
			...
			$("#like li").each(function(index){
				...
				$(this).jdLoad(function(){
					if (data["good"+(index+1)].nobespeak) {
						$li.find("div.fl").addClass("no");
					}
					$li.find("div.fl img")[0].src = "img/"+data["good"+(index+1)].src;
					$li.find("h3").html(data["good"+(index+1)].title);
					$li.find("div.fl").next().next("span").html(data["good"+(index+1)].description);
					$li.find("p>b").html(data["good"+(index+1)].price);
					$li.find("p>span").html(data["good"+(index+1)].oldprice);
					$li.find("p>i>span").html(data["good"+(index+1)].sales);
				});	

			});
			for(var pro in data){
				var index = data[pro].id;
				
					var $li = $("#like li").eq(index-1);
					$li.pro = pro;
				$li.jdLoad(function(){
					if (data[$li.pro].nobespeak) {
						$li.find("div.fl").addClass("no");
					}
					$li.find("div.fl img")[0].src = "img/"+data[$li.pro].src;
					$li.find("h3").html(data[$li.pro].title);
					$li.find("div.fl").next().next("span").html(data[$li.pro].description);
					$li.find("p>b").html(data[$li.pro].price);
					$li.find("p>span").html(data[$li.pro].oldprice);
					$li.find("p>i>span").html(data[$li.pro].sales);
				});		
			}
